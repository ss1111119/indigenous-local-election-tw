...
- 新增 `scripts/build_site_data.py`：由 `data/processed/` 的 summary 與 candidates 兩張長表產生兩份站台資料常數，**不連網、不手抄**。（站台沒有用到 votes 表——名錄不含得票數。）
...
- 該屆不存在的選舉種類標示為不存在，不以 `0` 填補：折線圖留空、表格寫「無此選舉」或「—」。（原文寫「以 `×` 標示（沿用既有慣例）」，站台從未渲染過 `×`，已更正——見 design 決策 5。）
