# site-heading-segmentation Specification

## Purpose

`docs/index.html` and `docs/legislative.html` are static Chinese pages with no
whitespace between words for the browser to find a natural line-break point.
Left alone, the browser's default CJK line-breaking allows a break between any
two characters, which can split a real word or phrase across two lines (for
example "投票率" or "地方") when the heading wraps on a narrow viewport.

This capability governs how the site generator inserts semantic word-break
points into those static headings at build time, using BudouX's Traditional
Chinese segmentation model, so that a wrap — if one occurs — always falls at a
real phrase boundary and never inside one. It covers: which pages and elements
are in scope, the requirement that inserting the break points must not alter
any visible text, idempotent rebuilds, and the CSS properties required for the
break points to actually take effect in the browser instead of being silently
overridden by the default per-character breaking rule.
