...
MUTATIONS = [
    ...
    # ---- 年齡：站台不得有第二份判準 ----
    ("站台重新自己實作年齡判準（不讀 年齡_有效）",
     '            int(c["年齡_有效"]) if c["年齡_有效"] else None,',
     '            (None if (c["年度"] in {"1994","1998","2002","2005","2006"}\n'
     '                      and c["年齡"] == "99") else int(c["年齡"])),'),
    ...
    ("契約：把實際會讀的 `鄉鎮市區` 從 candidates 清單移除",
     ...
     '        "號次", "姓名", "政黨代號", "政黨名稱", "性別", "年齡_有效", "現任",',
     '        "省市", "縣市", "選舉區", "行政區名稱",\n'
     '        "號次", "姓名", "政黨代號", "政黨名稱", "性別", "年齡_有效", "現任",'),
    ("契約：把實際會讀的 `政黨代號` 從 candidates 清單移除",
     '        "號次", "姓名", "政黨代號", "政黨名稱", "性別", "年齡_有效", "現任",',
     '        "號次", "姓名", "政黨名稱", "性別", "年齡_有效", "現任",'),
    ("契約：把實際會讀的 `年齡_有效` 從 candidates 清單移除",
     '"性別", "年齡_有效", "現任",',
     '"性別", "現任",'),
