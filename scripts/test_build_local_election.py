...
EXPECTED = {
    "rows": {"summary": 266336, "candidates": 7335, "votes": 1746316},
    "levels_summary": {'檔別合計': 33, '直轄市縣市': 242, '選舉區': 859, '鄉鎮市區': 4116, '投開票所': 176811, '村里': 84275},
    "levels_votes": {'選舉區': 6609, '鄉鎮市區': 22199, '投開票所': 1183724, '村里': 533784},
    "candidates_by_year_type": {'2022-T2': 73, '2022-T3': 80, '2022-D2': 20, '2022-R3': 92, '2022-R2': 134, '2022-T1': 1524, '2018-T2': 84, '2018-T3': 98, '2018-D2': 19, '2018-R3': 98, '2018-R2': 131, '2018-T1': 1569, '2014-T2': 77, '2014-T3': 79, '2014-D2': 20, '2014-R3': 94, '2014-R2': 118, '2014-T1': 1444, '2009-2010-T2': 69, '2009-2010-T3': 79, '2009-2010-T1': 1433},
    # 每屆每種選舉的全國數字。⚠️ 不可跨選舉種類相加——D2 與 R3 是同一批選民。
    "national": {
        "2009-2010-T1": {"選舉人數": 17335124, "投票數": 11883466, "候選人數": 1433,
                "當選人數": 843, "婦女保障當選人數": 15, "投票率": 68.55},
        "2009-2010-T2": {"選舉人數": 160576, "投票數": 89731, "候選人數": 69,
                "當選人數": 31, "婦女保障當選人數": 1, "投票率": 55.88},
        "2009-2010-T3": {"選舉人數": 170513, "投票數": 115572, "候選人數": 79,
                "當選人數": 32, "婦女保障當選人數": 0, "投票率": 67.78},
    ...
    # 版面：2022 為「男女合計」，其餘屆別為「合計在前」
    "layout_by_year": {k: sorted(v)[0] for k, v in {'2022': ['男女合計'], '2018': ['合計在前'], '2014': ['合計在前'], '2009-2010': ['合計在前']}.items()},
    # 當選註記四種值的分布。⚠️ 本專案曾宣稱資料中未出現 '-' 而未實際清點，
    # 擴充屆別後才發現 2014／2018 本來就有。分布釘死，不可再靜默漂移。
    "marks": {" ": 3285, "*": 3972, "!": 42, "-": 36},
    ...
    "known_turnout_anomalies": 1,   # 2010 區域議員 577/800=72.125 的進位臨界
    "known_vote_sum_anomalies": 1,  # 2009 區域議員 新崙里／竹仁里 3,106 票錯置
    "turnout_rows_checked": 205422,
...
def test_regression() -> None:
    ...
    for key, want in EXPECTED["national"].items():
        # 屆別鍵可能含連字號（如 2009-2010），只能從右切一刀
        year, etype = key.rsplit("-", 1)
    ...
    check("已知得票加總異常數", len(report["已知來源異常_得票加總"]),
          EXPECTED["known_vote_sum_anomalies"])
    check("得票加總異常的差額必為 0",
          [a["差額合計"] for a in report["已知來源異常_得票加總"]],
          [0] * EXPECTED["known_vote_sum_anomalies"])
