...
CUSTOM_ELECTION_TYPES = {
    ...
    # 山地鄉鄉長是官方 D1「鄉(鎮、市)長選舉」的【子集】——D1 涵蓋全部鄉鎮市長，
    # 山地鄉只是其中受地方制度法第 57 條第 2 項限制的那部分。沿用 D1 會讓
    # 長表的種類名稱與官方對照表不符，故另立自訂代碼。
    #
    # ⚠️ 登記為自訂代碼即代表 is_main_sequence() 為偽——這正是所要的：
    #    2014 年六個山地鄉改制為直轄市原住民區、改列 D2，D1-MT 由 30 掉到 24、
    #    D2 由 0 跳到 6。兩者接成一條線會被讀成席次變動，實際只是同一批單位
    #    換了職位分類（與 R2 在 2014 才納入是同型的誤讀，見 README）。
    "D1-MT": "山地鄉鄉長選舉",
}
...
ELECTION_TYPE_GRANULARITY = {
    ...
    "T-PRV3": "分平地山地",
    "T-COMBO": "合併未分平地山地",
    # 山地鄉鄉長不是「把原住民分到什麼程度」的問題——該職位依法限山地原住民
    # 擔任，選民則是全鄉選民，不分族別。
    "D1-MT": "不分平地山地（山地鄉）",
}
